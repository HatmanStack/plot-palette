    ...
    try:
        ...
        for item in response.get('Items', []):
            jobs.append({
                ...
                'cost_estimate': float(item.get('cost_estimate', 0.0)) if isinstance(item.get('cost_estimate'), str) else item.get('cost_estimate', 0.0),
                'budget_limit': float(item['budget_limit']) if isinstance(item.get('budget_limit'), str) else item.get('budget_limit', 0.0)
            })
